 ...
 *
 * z_pull — zenoh-pico pull-mode subscriber example for TI AM67A Cortex-R5F.
 *
 * Unlike z_sub (push model, callback driven), z_pull uses a ring-channel
 * so the application controls when to drain incoming samples.  The task
 * wakes every PULL_INTERVAL_MS and calls z_try_recv() in a loop until the
 * ring is empty.
 *
...
/* ---- Configuration -------------------------------------------------------- */
#define PULL_KEYEXPR     "t3/pub/**"
#define PULL_RING_SIZE   4U        /* ring buffer capacity (samples) */
#define PULL_INTERVAL_MS 5000U     /* how often to drain the ring */

/* ---- FreeRTOS task parameters --------------------------------------------- */
#define APP_TASK_NAME  "z_pull"
...
/* ---- Pull task ------------------------------------------------------------ */
static void z_pull_task(void *arg) {
    (void)arg;

    int net_rc = zenoh_net_init();
    if (net_rc != ZENOH_NET_OK) {
        DebugP_log("[z_pull] Network init failed (%d)\r\n", net_rc);
        vTaskDelete(NULL);
        return;
    }
    ...
    DebugP_log("[z_pull] IP: %s  mode: %s  keyexpr: %s\r\n",
               ip_str, ZENOH_MODE, PULL_KEYEXPR);
    ...
    if (z_open(&s, z_move(cfg), NULL) < 0) {
        DebugP_log("[z_pull] z_open failed\r\n");
        vTaskDelete(NULL);
        return;
    }
    ...
    if (zp_start_read_task(z_loan_mut(s), NULL) < 0 ||
        ...
        DebugP_log("[z_pull] Failed to start background tasks\r\n");
        z_drop(z_move(s));
        vTaskDelete(NULL);
        return;
    }

    /* Create ring channel: incoming samples are buffered here by the read task */
    z_owned_closure_sample_t closure;
    z_owned_ring_handler_sample_t handler;
    z_ring_channel_sample_new(&closure, &handler, PULL_RING_SIZE);

    z_view_keyexpr_t ke;
    z_view_keyexpr_from_str(&ke, PULL_KEYEXPR);

    z_owned_subscriber_t sub;
    if (z_declare_subscriber(z_loan(s), &sub, z_loan(ke), z_move(closure), NULL) < 0) {
        DebugP_log("[z_pull] z_declare_subscriber failed\r\n");
    ...
    DebugP_log("[z_pull] Polling '%s' every %u ms (ring size %u)...\r\n",
               PULL_KEYEXPR, (unsigned)PULL_INTERVAL_MS, (unsigned)PULL_RING_SIZE);

    while (1) {
        vTaskDelay(pdMS_TO_TICKS(PULL_INTERVAL_MS));
        ...
        if (res == Z_CHANNEL_NODATA) {
            DebugP_log("[z_pull] Nothing to pull — sleeping %u ms\r\n",
                       (unsigned)PULL_INTERVAL_MS);
        }
    }
...
void freertos_main(void *arg) {
    ...
    Drivers_open();
    Board_driversOpen();
    xTaskCreateStatic(z_pull_task, APP_TASK_NAME, APP_TASK_STACK,
